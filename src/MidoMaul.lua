...
function Utility:Padding(parent, left, top)
    ...
    p.PaddingRight = UDim.new(0, left or 10)
    p.PaddingTop = UDim.new(0, top or 0)
    p.PaddingBottom = UDim.new(0, top or 0)
    return p
end
...
function Library:Notify(title, text, duration)
    ...
    Utility:Create("TextLabel", {Parent=Toast, Text=title, TextColor3=Theme.Accent, Font=Theme.FontBold, TextSize=14, BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, TextXAlignment=Enum.TextXAlignment.Left})
    Utility:Create("TextLabel", {Parent=Toast, Text=text, TextColor3=Theme.Text, Font=Theme.Font, TextSize=12, BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, TextXAlignment=Enum.TextXAlignment.Left})
...
function Library:Window(options)
    ...
    local TitleLabel = Utility:Create("TextLabel", { Parent = Sidebar, Text = Title, TextColor3 = Theme.Accent, Font = Theme.FontBold, TextSize = 18, Size = UDim2.new(1, 0, 0, 50), BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left }); Utility:Padding(TitleLabel, 15)
    ...
    function WindowAPI:Tab(name)
        ...
        function Elements:Section(text)
            local Sec = Utility:Create("TextLabel", { Parent = Page, Text = string.upper(text), TextColor3 = Theme.SubText, Font = Theme.FontBold, TextSize = 11, Size = UDim2.new(1, 0, 0, 24), BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left }); Utility:Padding(Sec, 2, 10)
        end
        
        function Elements:Label(text)
            local Lab = Utility:Create("TextLabel", { Parent = Page, Text = text, TextColor3 = Theme.Text, Font = Theme.Font, TextSize = 13, Size = UDim2.new(1, 0, 0, 24), BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped=true }); Utility:Padding(Lab, 2)
        end
        ...
        function Elements:Keybind(options, callback)
            ...
            UserInputService.InputBegan:Connect(function(i) if Binding and i.UserInputType == Enum.UserInputType.Keyboard then Binding = false; Library.Keys[Flag].Key = i.KeyCode.Name; Btn.Text = i.KeyCode.Name; Utility:Tween(Stroke, {Color = Theme.Stroke}) end end)
        end
        ...
        function Elements:ConfigSystem()
            ...
            Btn("Save", function() Library:SaveConfig(Name) end)
            Btn("Load", function() Library:LoadConfig(Name) end)
            Btn("List", function() Library:Notify("Configs", table.concat(Library:GetConfigs(), ", ")) end)
        end
