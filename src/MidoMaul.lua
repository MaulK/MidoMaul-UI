--[[
    MIDOMAUL // v6.1 CUSTOMIZATION UPDATE
    - Added: Live Accent Color Changing (Library:SetAccent)
    - Added: Library:Unload() for clean cleanup
    - Updated: All components register their accent elements automatically
]]
...
local Theme = {
    ...
    Accent      = Color3.fromRGB(0, 200, 255), -- Default Cyan
...
local Library = { 
    Flags = {}, 
    Items = {}, 
    Accents = {}, -- Stores objects that need color updates
    Connections = {}, -- Stores connections to clean up
...
function Utility:Padding(parent, left, top)
    local p = Instance.new("UIPadding")
    p.Parent = parent
    p.PaddingLeft = UDim.new(0, left or 10); p.PaddingRight = UDim.new(0, left or 10)
    p.PaddingTop = UDim.new(0, top or 0); p.PaddingBottom = UDim.new(0, top or 0)
    return p
end
...
function Utility:RegisterAccent(obj, prop)
    ...
    obj[prop] = Theme.Accent -- Set initial color
end
...
function Library:SetAccent(color)
    ...
    for _, item in pairs(Library.Accents) do
        if item.Object then
            Utility:Tween(item.Object, {[item.Property] = color}, 0.2)
        end
    end
end
...
function Library:Notify(title, text, duration)
    ...
    Utility:RegisterAccent(Utility:Create("Frame", {Parent=Toast, Size=UDim2.new(0,2,1,0), Position=UDim2.new(0,-6,0,0), BorderSizePixel=0}), "BackgroundColor3")
    Utility:Create("TextLabel", {Parent=Toast, Text=title, TextColor3=Theme.Text, Font=Theme.FontBold, TextSize=14, BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, TextXAlignment=Enum.TextXAlignment.Left})
    Utility:Create("TextLabel", {Parent=Toast, Text=text, TextColor3=Theme.SubText, Font=Theme.Font, TextSize=12, BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y, TextXAlignment=Enum.TextXAlignment.Left})
    task.delay(duration or 3, function() Toast:Destroy() end)
end

function Library:Unload()
    for _, conn in pairs(Library.Connections) do conn:Disconnect() end
    if CoreGui:FindFirstChild("MidoMaul_Main") then CoreGui.MidoMaul_Main:Destroy() end
    Library.Flags = {}
    Library.Items = {}
    Library.Accents = {}
end
...
function Library:Window(options)
    ...
    Library.Toggle = ToggleUI; OpenBtn.MouseButton1Click:Connect(ToggleUI)
    table.insert(Library.Connections, UserInputService.InputBegan:Connect(function(i, gp) if not gp and i.KeyCode == ToggleKey then ToggleUI() end end))
    
    local CloseBtn = Utility:Create("TextButton", { Parent = Main, BackgroundTransparency = 1, Size = UDim2.new(0, 30, 0, 30), Position = UDim2.new(1, -35, 0, 10), Text = "×", TextColor3 = Theme.SubText, Font = Theme.Font, TextSize = 24 }); CloseBtn.MouseButton1Click:Connect(ToggleUI)
    ...
    function WindowAPI:Tab(name)
        ...
        local Page = Utility:Create("ScrollingFrame", { Parent = PageHolder, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Visible = false, ScrollBarThickness = 2, ScrollBarImageColor3 = Theme.Accent, CanvasSize = UDim2.new(0,0,0,0), BorderSizePixel=0 }); Utility:Create("UIPadding", {Parent=Page, PaddingRight=UDim.new(0,10)})
        ...
        local Elements = {}
        function Elements:Section(text) local Sec = Utility:Create("TextLabel", { Parent = Page, Text = string.upper(text), TextColor3 = Theme.SubText, Font = Theme.FontBold, TextSize = 11, Size = UDim2.new(1, 0, 0, 24), BackgroundTransparency = 1, TextXAlignment = Enum.TextXAlignment.Left }); Utility:Padding(Sec, 2, 10) end
        ...
        function Elements:Button(text, callback)
            local Btn = Utility:Create("TextButton", { Parent = Page, BackgroundColor3 = Theme.Element, Size = UDim2.new(1, 0, 0, 36), Text = "", AutoButtonColor = false }); Utility:Corner(Btn); Utility:Stroke(Btn, Theme.Stroke)
        ...
        function Elements:Toggle(options, callback)
            ...
            local Tgl = Utility:Create("TextButton", { Parent = Page, BackgroundColor3 = Theme.Element, Size = UDim2.new(1, 0, 0, 36), Text = "", AutoButtonColor = false }); Utility:Corner(Tgl); Utility:Stroke(Tgl, Theme.Stroke)
            ...
            local Check = Utility:Create("Frame", { Parent = Box, BackgroundColor3 = Theme.Accent, Size = UDim2.new(1, -4, 1, -4), Position = UDim2.new(0, 2, 0, 2), Transparency = 1 }); Utility:Corner(Check, UDim.new(0,3)); Utility:RegisterAccent(Check, "BackgroundColor3")
        ...
        function Elements:Keybind(options, callback)
            ...
            table.insert(Library.Connections, UserInputService.InputBegan:Connect(function(i) if Binding and i.UserInputType == Enum.UserInputType.Keyboard then Binding = false; Library.Keys[Flag].Key = i.KeyCode.Name; Btn.Text = i.KeyCode.Name; Utility:Tween(Stroke, {Color = Theme.Stroke}) end end))
        end
        function Elements:ColorPicker(options, callback)
            ...
            local R = CreateBox(UDim2.new(0,0,0.35,0), Color3.fromRGB(255,100,100), "255"); local G = CreateBox(UDim2.new(0.35,0,0.35,0), Color3.fromRGB(100,255,100), "255"); local B = CreateBox(UDim2.new(0.7,0,0.35,0), Color3.fromRGB(100,100,255), "255")
            local function Update() local c = Color3.fromRGB(tonumber(R.Text) or 0, tonumber(G.Text) or 0, tonumber(B.Text) or 0); Preview.BackgroundColor3 = c; Library.Flags[Flag] = c; pcall(callback, c) end
            R.FocusLost:Connect(Update); G.FocusLost:Connect(Update); B.FocusLost:Connect(Update)
            local Open = false; Preview.MouseButton1Click:Connect(function() Open = not Open; Picker.Visible = Open; Frame:TweenSize(Open and UDim2.new(1, 0, 0, 120) or UDim2.new(1, 0, 0, 36), "Out", "Quart", 0.3, true) end)
        end
        function Elements:ProgressBar(options)
             ...
             local PFrame = Utility:Create("Frame", { Parent = Page, BackgroundColor3 = Theme.Element, Size = UDim2.new(1, 0, 0, 40) }); Utility:Corner(PFrame); Utility:Stroke(PFrame, Theme.Stroke)
